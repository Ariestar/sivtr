...
公开服务只接收 AES-256-GCM 密文。解密密钥位于链接 fragment，不会随 HTTP 请求发送给托管服务；因此查看者无需账号，发布者设备离线也不影响查看。链接持有者均可阅读，默认 7 天失效（可选 2 小时、1 天、3 天、30 天；旧版 90 天链接仍可读取），可以在本机用 `sivtr publish revoke` 提前撤销。管理凭据只保存在独立的 `publication-state.db`；数据库丢失时 v1 没有账号恢复路径。
