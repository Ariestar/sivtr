...
`publish preview` 的原子选择只作用于浏览器快照，不会改变 `share` 暴露的 workspace，也不会把 remote/group 内容变成可发布输入。需要按 User、Assistant、Tool、Skill、Thinking 原子挑选时，请先阅读 [发布浏览器只读对话链接](/zh-cn/usage/publish/)；需要跨设备检索时，继续按本页的 share/invite/remote 流程操作。
