...
这些键仅在浏览 Codex / AI 对话时可用。

| 按键 | 动作 |
| --- | --- |
| `m` | 复制当前高亮对话的 `## User` 正文，不包含标题行 |
| `2:8m` | 按当前高亮完整对话的 1-based 内容行号复制一个范围 |
| `n` | 复制第一个 `## Assistant` 标题下面的全部内容，不包含标题行 |
| `Backspace` | 在按 `m` 之前删除待提交的数字范围 |
| `Esc` | 先清除待提交的数字范围或错误状态，再恢复普通返回行为 |
