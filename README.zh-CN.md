...
| `sivtr serve` | 把当前 workspace 的 session 以只读 HTTP 方式提供给远程设备。 |
...
在持有 workspace 的那台设备上启动只读 server（不传 `--token` 会自动生成）：

```bash
sivtr serve                 # 仅 localhost；加 --lan 暴露到局域网
```
...
`sivtr serve` 是 opt-in、默认 localhost、强制 bearer token、只读，并在数据离开本机前脱敏常见密钥（API key、token、PEM 私钥）。未登记的别名会报错——用 `sivtr remote add` 登记。
