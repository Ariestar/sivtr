...
- **跨设备访问**：把某个 workspace 以只读方式 serve 出去，用 `desk://...` ref 像读本地一样浏览另一台设备的 session——用于协同开发。
...
| `sivtr serve` / `sivtr pair` | 把某个 workspace 的 session 以只读 HTTP 方式提供给远程设备配对。 |
...
在持有 workspace 的那台设备上启动只读 server（`sivtr pair`，也可用 `sivtr serve`）：

```bash
sivtr pair                    # 仅 localhost；交互选 workspace；自动生成 token
sivtr pair -w <key>           # 指定要开放的 workspace（按 key）
sivtr pair --lan              # 暴露到局域网
```

在另一台设备上用 SSH 风格的目标串登记它，然后用 `<别名>://` ref（和普通 ref 用法完全一样）：

```bash
sivtr remote add desk@192.168.1.20      # 端口默认 7421；token 会提示输入
sivtr show desk://terminal/session_42/3/o/1
```

`sivtr serve` / `sivtr pair` 是 opt-in、默认 localhost、强制 bearer token、只读，并在数据离开本机前脱敏常见密钥（API key、token、PEM 私钥）。未登记的别名会报错——用 `sivtr remote add` 登记。
