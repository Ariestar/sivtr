...
- **可选 MCP server**（`sivtr mcp serve`，用 `sivtr mcp install` 一键注册）：给 agent 宿主提供结构化工具调用（search / show / zoom / filter / status）。
...
| `sivtr workspace` / `sivtr wb` | 列出本机已知 workspace（`name:body` 的 origin 标签）。 |
